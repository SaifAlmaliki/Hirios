...
serve(async (req) => {
  ...
  try {
    ...
    if (!emailConfigs || emailConfigs.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: 'No active email configurations found' }),
...
async function processEmailAccount(config: EmailConfig): Promise<number> {
  ...
  try {
    ...
    try {
      ...
      for await (let message of client.search(searchQuery, { uid: true })) {
        try {
          ...
          const { data: existingEmail } = await supabase
            ...
            .eq('email_uid', message.uid.toString())
            .single()
          ...
          // Download the email
          const emailData = await client.download(message.uid, { uid: true })
          ...
          if (parsed.attachments && parsed.attachments.length > 0) {
            for (const attachment of parsed.attachments) {
              if (attachment.contentType === 'application/pdf' || 
                ...
                const resumeData = await processResumeAttachment(
                  attachment,
                  config.company_profile_id,
                  config.id
                )
...
async function processResumeAttachment(
  attachment: any, 
...
async function sendToWebhook(resumeRecord: any, fileContent: Uint8Array): Promise<void> {
  try {
