...
-- Using a real user ID instead of the dummy UUID
...
-- Get a real user ID from the applications table
-- We'll use the uploaded_by_user_id from the applications
SELECT DISTINCT
...
SELECT DISTINCT
    ...
    sr.job_id,
    'screened' as status,
    a.uploaded_by_user_id, -- Use the real user ID from applications
    sr.created_at,
    NOW() as updated_at
FROM screening_results sr
...
WHERE sr.job_id = '9c34430d-b76b-4c17-a7d4-ccf573a4f376'
  ...
  AND a.uploaded_by_user_id IS NOT NULL
